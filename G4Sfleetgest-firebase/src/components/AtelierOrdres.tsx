      ...
      <div className="flex flex-col gap-4 rounded-xl bg-white p-6 shadow-sm print:hidden sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un ordre, un véhicule, un mécanicien…" className="w-64 rounded-lg border border-slate-300 py-2 pl-9 pr-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
          </div>
          ...
          <button onClick={() => { setEditOrderId(''); setShowForm(true); }} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700"><Plus className="h-4 w-4" /> Nouvel ordre</button>
        </div>
      </div>
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4 p-6">
          <label className="block text-xs font-medium text-slate-600">Véhicule
            <select required value={f.vehicleId} onChange={e => handleVehicleChange(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500">
          ...
          <label className="block text-xs font-medium text-slate-600">Mécanicien
            <select value={f.mechanic} onChange={e => handleMechanicChange(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500">
          ...
          <label className="col-span-2 block text-xs font-medium text-slate-600">Motif de l'intervention<input required value={f.issue} onChange={e => up('issue', e.target.value)} placeholder="Ex. Révision, panne, contrôle…" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" /></label>
          <label className="block text-xs font-medium text-slate-600">Coût (FCFA)<input type="number" min="0" value={f.cost} onChange={e => up('cost', Number(e.target.value))} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" /></label>
          <label className="block text-xs font-medium text-slate-600">Priorité
            <select value={f.priority} onChange={e => up('priority', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500">
              {ORDER_PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </label>
          <label className="block text-xs font-medium text-slate-600">Date de début<input type="date" required value={f.startDate} onChange={e => up('startDate', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" /></label>
          <label className="block text-xs font-medium text-slate-600">Date de fin<input type="date" required value={f.endDate} onChange={e => up('endDate', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" /></label>
          <label className="block text-xs font-medium text-slate-600">Heure de début<input type="time" value={f.startTime} onChange={e => up('startTime', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" /></label>
          <label className="block text-xs font-medium text-slate-600">Statut
            <select value={f.status} onChange={e => up('status', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500">
              {ORDER_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </label>
          {f.status === 'Terminé' && (
            <label className="block text-xs font-medium text-slate-600">Heure de fin<input type="time" value={f.endTime} onChange={e => up('endTime', e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500" /></label>
          )}
          ...
          <div className="col-span-2 flex items-center justify-between border-t pt-4">
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" className="rounded-lg bg-emerald-600 px-5 py-2 text-sm font-semibold text-white hover:bg-emerald-700">{order ? 'Mettre à jour' : "Créer l'ordre"}</button>
            </div>
          </div>
        </form>
      </div>
